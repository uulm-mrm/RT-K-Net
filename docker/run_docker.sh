...
if [ "$SHOW_HELP" = true ]; then
  ...
  echo " * -s|--bind_sources:    Bind local sources into docker container (Only supported in dev container)"
...
if [ "$BIND_ZSH" = true ]; then
  DOCKER_ARGS+=(
    ...
    -v $HOME/.vimrc:/home/appuser/.vimrc
    -v $HOME/.viminfo:/home/appuser/.viminfo
    -v $HOME/.vim:/home/appuser/.vim
...
if [ "$BIND_SOURCES" = true ]; then
  ...
  DOCKER_ARGS+=(
    -v $PROJECT_DIR:/home/appuser/RT-K-Net
  )
fi
