...
if [ "$BUILD_DEV_CONTAINER" = true ]; then
  if [[ "$(docker images -q rtknet:${TAG} 2> /dev/null)" == "" ]]; then
    echo "Base image missing. Build base image first..."
    docker build -t rtknet:${TAG} .
  fi
  docker build --build-arg TAG=${TAG} --build-arg USER_ID=$(id -u) --build-arg GROUP_ID=$(id -g) -t rtknet-dev:${TAG} -f develop.Dockerfile .
else
  docker build -t rtknet:${TAG} .
fi
